...
graph TD
    A[贝尔曼方程] --> B{技术路径选择}
    B -->|路径1| C[Value-Based<br/>Q-Learning/DQN]
    B -->|路径2| D[Policy-Based<br/>Policy Gradient]
    
    C --> E[评估每个动作的价值<br/>然后选最大的]
    D --> F[直接优化策略参数<br/>输出动作分布]
...
```mermaid
graph LR
    A[贝尔曼方程<br/>1950s] --> B[Q-Learning<br/>1989]
    B --> C[DQN<br/>2013]
...
```mermaid
graph TD
    A["策略梯度理论<br/>Williams 1992"] --> B["REINFORCE<br/>高方差问题"]
    B --> C["Actor-Critic<br/>引入Baseline"]
    C --> D["A3C/A2C<br/>异步并行 2016"]
...
```mermaid
graph LR
    A[RNN推荐<br/>GRU4Rec 2016] --> B[Self-Attention<br/>SASRec 2018]
...
graph TD
    ...
    Q1 -->|"< 100"| V1["✅ 可以使用 Q-Learning"]
    Q1 -->|"100-10,000"| V2["✅ 可以尝试 DQN"]
    Q1 -->|"> 10,000"| Q2{"是否需要生成序列?"}
    
    Q2 -->|"否<br/>单次推荐"| P1["推荐: Actor-Critic"]
    Q2 -->|"是<br/>推荐列表"| Q3{"是否有预训练模型?"}
    
    Q3 -->|"否"| P2["推荐: PPO从头训练"]
    Q3 -->|"是<br/>有监督数据"| Q4{"计算资源?"}
    
    Q4 -->|"有限"| P3["推荐: GRPO<br/>（无需Critic）"]
    Q4 -->|"充足"| P4["推荐: ECPO + Transformer<br/>（生成式推荐）"]
...
```mermaid
graph TD
    L1["第1-3章<br/>RL基础理论"] --> L2["阶段1: 理解两大路径"]
    L2 --> L3["阶段2: 策略梯度方法"]
    L3 --> L4["阶段3: Transformer架构"]
    L4 --> L5["阶段4: 生成式RL"]
    L5 --> L6["阶段5: 推荐系统应用"]
    
    L2 -.必读论文.-> P1["DQN (Mnih+ 2015)"]
    L3 -.必读论文.-> P2["PPO (Schulman+ 2017)"]
    L4 -.必读论文.-> P3["Attention is All You Need"]
    L5 -.必读论文.-> P4["GRPO (DeepSeek 2024)"]
    L6 -.必读论文.-> P5["OneRec (Kuaishou 2025)"]
...
graph LR
    A[贝尔曼方程<br/>1950s] --> B[Q-Learning<br/>1989]
    B --> C[DQN<br/>2013]
    C -.卡在这里.-> D1[推荐系统的尝试<br/>2015-2018]
...
```mermaid
graph TB
    subgraph Theory["基础理论层 (01-03章)"]
        A["RL核心概念<br/>Agent-Environment-Reward"]
        B["MDP<br/>⟨S,A,P,R,γ⟩"]
    ...
    subgraph ValueBased["Value-Based 路径 (推荐系统受阻)"]
    ...
    subgraph PolicyBased["Policy-Based 路径 (推荐系统主流)"]
...
timeline
    ...
    1950s : 贝尔曼方程 : 动态规划理论基础
    1989 : Q-Learning : Watkins博士论文
    1992 : REINFORCE : Williams策略梯度
    2000 : Actor-Critic : Konda & Tsitsiklis
    2013 : DQN突破 : Atari游戏 : Mnih et al.
    2015 : TRPO : 信任域优化 : Schulman et al.
    2016 : A3C : 异步优势Actor-Critic
    2017 : PPO : OpenAI主推算法
    2017 : Transformer : Attention is All You Need
    2018 : BERT/GPT : 预训练语言模型
    2019 : BERT4Rec : Transformer用于推荐
    2022 : ChatGPT : RLHF证明策略梯度威力
    2024 : GRPO : DeepSeek优化大模型微调
    2025 : OneRec : 快手生成式推荐落地
```
